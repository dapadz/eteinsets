...
### 1. `systemBarsPadding` и `systemBarsMargin` — Работа с системными панелями

Это базовые эффекты для реализации **edge-to-edge**.

- `systemBarsPadding(...)` добавляет отступы системных панелей к `padding` View.
- `systemBarsMargin(...)` добавляет те же значения к `margin` View.

Первый вариант удобен, когда нужно защитить внутренний контент, второй — когда нужно сместить саму View относительно родителя.
...
Частая проблема: при открытии клавиатуры возникает двойной отступ снизу (от системной панели + от клавиатуры). Модификатор `.hideWhenIme()` плавно убирает нижний отступ системной панели, когда появляется клавиатура.

Модификатор работает одинаково и для `systemBarsPadding(...)`, и для `systemBarsMargin(...)`.
...
```kotlin
fab.insets {
    // То же самое поведение, но через marginBottom
    systemBarsMargin(bottom = true).hideWhenIme()
}
```
